...
export default function BlogCard({ post, variant = 'light', className }: Props) {
    ...
    if (variant === 'hero') {
        return (
            ...
            >
                <Image fill src={post.cover} alt={post.title} className="object-cover" priority />
    ...
    if (variant === 'dark') {
        return (
            ...
            >
                <Image fill src={post.cover} alt={post.title} className="object-cover" />
    ...
    return (
        <Link
            href={href}
            className={clsx(
                'rounded-2xl border bg-white hover:shadow-md transition block',
